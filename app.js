...
const queen = '<i class="fas fa-chess-queen" style="color: #fff"></i>';
...
let array = [0, 2, 1, 1, 2, 10, 4, 40, 92];
...
class Queen {
  ...
  isValid = async (board, r, col, n) => {
    ...
    for (let i = r - 1; i >= 0; --i) {
      ...
      if (value == queen) {
        column.style.backgroundColor = attacking_queen;
        currentColumn.innerHTML = "-";
        currentColumn.style.color = dash_color;
        column.style.color = dash_color;
        return false;
      }
      column.style.backgroundColor = queen_cannot_be_placed_here;
      currentColumn.style.color = dash_color;
      column.style.color = dash_color;
      await q.delay();
    }
      ...
      for (let k = 0; k < n; ++k)
        (j + k) & 1
          ? (row.getElementsByTagName("td")[k].style.backgroundColor =
            dark_board_color) // board dark color
          : (row.getElementsByTagName("td")[k].style.backgroundColor =
            light_board_color); // board light color
    }
  };
  ...
  for (let k = 0; k < array[n]; ++k) {
    ...
    for (let i = 0; i < n; ++i) {
      ...
      for (let j = 0; j < n; ++j) {
        ...
        col.innerHTML = "-";
        col.style.border = board_border; // ? board border
      }
    }
